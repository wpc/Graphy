var Graphy = function(x, y, w, h) {
  ...
  var springs = [];
  var maxIters = 200;
  var ideaIterationIterval = 20 //ms
  var damping = 0.500;
  ...
  var public =  {
    ...
    chargeConst: function() {
      return 10000000 / Math.sqrt(nodes.length);
    },
...
Graphy.Node = function(graph, paper, name) {
  ...
  var public = {
    ...
    coulombRepulsionXTo: function(anotherNode) {
      return graph.chargeConst() * (x - anotherNode.x()) / Math.pow(distance(anotherNode), 3)
    },
    
    coulombRepulsionYTo: function(anotherNode) {
      return graph.chargeConst() * (y - anotherNode.y()) / Math.pow(distance(anotherNode), 3)
    },
