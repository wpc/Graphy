var Graphy = function(x, y, w, h) {
  ...
  var springs = [];
  var maxIters = 300;
  var iterationIterval = 25 //ms
  var damping = 0.500;
  var minusAverageKineticEnergy = 2.5;
  ...
  var updateNodes = function() {
    ...
    $.map(nodes, function(node){
      node.actOnForce(iterationIterval);
    })
  ...
  var totalAverageKineticEnergy = function() {
    var result = 0;
    $.map(nodes, function(node) {
  ...
  var drawIteration = function(currentIteration) {
    ...
    if (totalAverageKineticEnergy() < minusAverageKineticEnergy ) {
      drawIteration(0);
      return;
    }
    
    setTimeout(function() {
      drawIteration(currentIteration - 1);
    }, iterationIterval);
  ...
  var public =  {
    ...
    draw: function() {
      chargeConst = 10000000 / Math.pow(nodes.length, 0.25);
